...
- **Multi-select bulk edit** — long-press any row to enter selection mode, tap rows to (de)select (**Shift+click** selects the whole range in between), then apply one change to all at once from the floating bar: set status, add a project, set a due date / priority / category, or delete. `全選` selects the whole board; `Esc` (or the ✕) exits.
- **Subtask grouping** — within a status column, a parent's subtasks are clustered under a single collapsible header (`↳ 主任務 · n`) with an indented connector rail, so it's clear at a glance which subtasks belong to which parent instead of each row repeating the parent's name. The header name jumps to the parent; the collapse state is remembered per browser.
...
- **AI 整理任務** — the topbar 整理 button sends a long braindump to the Claude API, which returns a structured set of parent tasks + subtasks (title, description, category, priority) to review, edit, and create in one go. Requires `ANTHROPIC_API_KEY` (and optional `ANTHROPIC_MODEL`); without the key the button returns a clear "not configured" message.
- **AI 產生開發需求** — multi-select some tasks, then `產生需求` on the bulk bar asks Claude to consolidate them (with their descriptions, projects, categories, priorities and parent/subtask context) into one developer-ready requirements brief (overview / itemised requirements with acceptance notes / affected modules / implementation notes / suggested order). Review or edit it, then copy it to hand to a coding agent like Claude Code. Same `ANTHROPIC_API_KEY` / `ANTHROPIC_MODEL`.
...
- **Manual archive** — for a task that never needs a deploy (a local-only dev chore), 封存 it directly from the task editor, or archive several at once from the multi-select bar. It moves to the archive status (stamped `archived_at`) and lands in 部署歷史; 取消封存 sends it back to the board. Both offer undo.
