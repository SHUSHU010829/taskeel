...
- **Multi-select bulk edit** — long-press any row to enter selection mode, tap rows to (de)select, then apply one change to all at once from the floating bar: set status, add a project, set a due date / priority / category, or delete. `全選` selects the whole board; `Esc` (or the ✕) exits. Subtask rows show their parent task's title inline.
